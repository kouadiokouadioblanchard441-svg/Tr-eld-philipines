    ...
    <div className="space-y-4">
      {pendingCount > 0 && (
        <div className="bg-yellow-50 dark:bg-yellow-950 border border-yellow-200 dark:border-yellow-800 rounded-xl p-3 flex items-center gap-2">
          ...
          <p className="text-sm font-medium text-yellow-700 dark:text-yellow-300">
             {pendingCount} dépôt{pendingCount > 1 ? "s" : ""} en attente d'approbation
          </p>
        </div>
      )}
                ...
                <CardContent className="p-4 space-y-3">
                  ...
                  {(deposit.status === "pending" || deposit.status === "processing") && (
                    <div className="flex gap-2">
                      ...
                      >
                        {processingId === deposit.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Check className="w-4 h-4 mr-1" />Approuver</>}
                      </Button>
                      <Button
                        size="sm"
                        variant="outline"
      ...
      <Dialog open={!!screenshotModal} onOpenChange={() => setScreenshotModal(null)}>
        <DialogContent className="max-w-lg">
          ...
          {screenshotModal && (
            <img src={screenshotModal} alt="Capture d'écran" className="w-full rounded-xl object-contain max-h-[70vh]" />
          )}
        </DialogContent>
      </Dialog>
    </div>
