      ...
      <CardContent className="p-3">
        ...
        <div className="mt-2 pt-2 border-t">
          <p className="text-sm font-medium text-primary">
            Total investi: {member.totalInvested.toLocaleString()} GPB
          </p>
            ...
            <div className="mt-1">
              ...
              {member.products.map((p, i) => (
                <p key={i} className="text-xs">
                  - {p.productName} ({p.productPrice.toLocaleString()} GPB)
            ...
            <Tabs defaultValue="level1" className="w-full">
              ...
              <TabsContent value="level1" className="mt-4">
                <Card className="mb-3">
                  <CardContent className="p-3 text-center">
                    <p className="text-lg font-bold text-primary">
                      {teamData.totalLevel1Invested.toLocaleString()} GPB
                    </p>
                ...
                {teamData.level1.length > 0 ? (
                  teamData.level1.map(member => (
                    ...
                    <p className="text-center text-muted-foreground py-4">Aucun parrainage de niveau 1</p>
                )}
              </TabsContent>
              ...
              <TabsContent value="level2" className="mt-4">
                <Card className="mb-3">
                  <CardContent className="p-3 text-center">
                    <p className="text-lg font-bold text-primary">
                      {teamData.totalLevel2Invested.toLocaleString()} GPB
                    </p>
                ...
                {teamData.level2.length > 0 ? (
                  teamData.level2.map(member => (
                    ...
                    <p className="text-center text-muted-foreground py-4">Aucun parrainage de niveau 2</p>
                )}
              </TabsContent>
              ...
              <TabsContent value="level3" className="mt-4">
                <Card className="mb-3">
                  <CardContent className="p-3 text-center">
                    <p className="text-lg font-bold text-primary">
                      {teamData.totalLevel3Invested.toLocaleString()} GPB
                    </p>
        ...
        <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
          ...
          {selectedUser && (
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <div>
                   <label className="text-sm font-medium">Mettre à jour le solde</label>
                      ...
                      <SelectContent>
                        {products?.filter(p => !p.isFree).map((product) => (
                          <SelectItem key={product.id} value={product.id.toString()}>
                            {product.name} - {product.price.toLocaleString()} GPB
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                      ...
                      userProducts.map((up) => (
                        <div key={up.id} className={`flex items-center justify-between p-2 rounded-lg ${up.isActive ? "bg-green-500/10 border border-green-500/20" : "bg-secondary"}`}>
                          <div>
                            ...
                            {up.productPrice.toLocaleString()} GPB - Jour {up.daysClaimed}/{up.totalCycle}
