    ...
    <div className="space-y-4">
        ...
        <TabsContent value="products" className="space-y-4 mt-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="text-sm text-muted-foreground">Nombre à créer :</span>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="grid grid-cols-3 gap-2 text-sm">
                            <div className="bg-secondary/50 rounded-lg p-2 text-center">
                              ...
                              <p className="font-bold text-primary">{sp.price.toLocaleString()} GPB</p>
                            </div>
                            <div className="bg-secondary/50 rounded-lg p-2 text-center">
                              <p className="text-muted-foreground text-xs">Rendement</p>
                              <p className="font-bold text-green-600">{sp.returnAmount.toLocaleString()} GPB</p>
                            </div>
                            <div className="bg-secondary/50 rounded-lg p-2 text-center">
        ...
        <TabsContent value="stakings" className="space-y-4 mt-4">
          <div className="grid grid-cols-3 gap-2">
            ...
            <div className="bg-secondary rounded-xl p-3 text-center">
              ...
              <p className="font-bold text-orange-600">{totalStaked.toLocaleString()} GPB</p>
            </div>
            <div className="bg-secondary rounded-xl p-3 text-center">
              <p className="text-xs text-muted-foreground">Total libéré</p>
              <p className="font-bold text-green-600">{totalReleased.toLocaleString()} GPB</p>
            </div>
          </div>
                    ...
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">
                          {s.amountPaid.toLocaleString()} GPB → {s.returnAmount.toLocaleString()} GPB · Release: {formatDate(s.releaseDate)}
                        </p>
                      </div>
      ...
      <Dialog open={showForm} onOpenChange={closeForm}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {editTarget ? "Modifier le produit de placement" : forms.length > 1 ? `Ajouter ${forms.length} produits de placement` : "Ajouter un produit de placement"}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-6">
            {forms.map((form, i) => (
              <div key={i} className={`space-y-3 ${forms.length > 1 ? "border border-border rounded-xl p-4" : ""}`}>
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div>
                    <label className="text-sm font-medium">Price (GPB)</label>
                  ...
                  <div>
                    <label className="text-sm font-medium">Return (GPB)</label>
