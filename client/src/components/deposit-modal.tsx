...
const depositSchema = z.object({
  ...
  paymentChannelId: z.string().min(1, "Le canal de recharge est obligatoire"),
});
...
export default function DepositModal({ open, onClose }: DepositModalProps) {
  ...
  const handleCustomAmount = () => {
    ...
    if (amount >= 2000) {
      ...
      toast({ title: "Montant invalide", description: "Le montant minimum est indiqué en GPB", variant: "destructive" });
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {step === "amount" ? "Recharger" : "Détails du paiement"}
          </DialogTitle>
        </DialogHeader>
