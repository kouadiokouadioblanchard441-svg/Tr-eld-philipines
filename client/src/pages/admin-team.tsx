...
export default function AdminTeamPage() {
  ...
  const { data: teamData, isLoading: teamLoading } = useQuery<DetailedTeam>({
    ...
    queryFn: async () => {
      ...
      if (!res.ok) throw new Error("Impossible de récupérer l'équipe");
      return res.json();
    },
    enabled: !!userId,
  });
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-primary">
                {member.totalInvested.toLocaleString()} GPB
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-1">
              {member.products.map((p, i) => (
                <div key={i} className="flex items-center justify-between text-sm bg-secondary rounded-lg px-3 py-2">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-medium">{p.productPrice.toLocaleString()} GPB</span>
      ...
      <CardContent className="p-4">
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <p className="text-xs text-muted-foreground">Total investi</p>
            <p className="text-xl font-bold text-primary">{total.toLocaleString()} GPB</p>
          </div>
        </div>
      </CardContent>
    </Card>
              ...
              <CardContent className="p-4">
                ...
                <div className="mt-4 pt-4 border-t text-center">
                  ...
                  <p className="text-2xl font-bold text-primary">
                    {(teamData.totalLevel1Invested + teamData.totalLevel2Invested + teamData.totalLevel3Invested).toLocaleString()} GPB
                  </p>
                </div>
              </CardContent>
            </Card>
