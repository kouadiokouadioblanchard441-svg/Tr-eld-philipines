...
function StatusBadge({ status }: { status: string }) {
  ...
  const labels: Record<string, string> = {
    pending: "En attente", approved: "Approuvé", rejected: "Rejeté", processing: "En cours",
  };
  return (
  ...
  const depositMutation = useMutation({
    ...
    onSuccess: (_, vars) => {
      queryClient.invalidateQueries({ queryKey: ["/api/banker/deposits"] });
       toast({ title: vars.action === "approve" ? "Dépôt approuvé !" : "Dépôt rejeté" });
    },
  ...
  const withdrawalMutation = useMutation({
    ...
    onSuccess: (_, vars) => {
      queryClient.invalidateQueries({ queryKey: ["/api/banker/withdrawals"] });
       toast({ title: vars.action === "approve" ? "Retrait approuvé !" : "Retrait rejeté et remboursé" });
    },
       onError: (e: any) => toast({ title: "Erreur", description: e.message, variant: "destructive" }),
  });
          ...
          <TabsContent value="deposits" className="space-y-3">
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <Input
                   placeholder="Rechercher un nom, téléphone ou référence..."
                  value={depositSearch}
                  ...
                  <Card key={deposit.id} className={deposit.status === "pending" ? "border-yellow-500/30" : ""}>
                    <CardContent className="p-4">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-sm bg-secondary/50 rounded-lg p-3 mb-3">
                        ...
                        <div>
                           <p className="text-muted-foreground text-xs">Numéro du payeur</p>
                       ...
                       {(deposit as any).screenshot && (
                         <div className="mb-3">
                           ...
                           <p className="text-xs font-medium text-muted-foreground">Capture du paiement</p>
                           </div>
                           <button
                             type="button"
            ...
            <div className="flex gap-2 overflow-x-auto pb-1">
              {(["pending", "approved", "rejected", "processing", "all"] as const).map(s => (
                <Button key={s} size="sm" variant={withdrawalStatus === s ? "default" : "outline"}
                  ...
                  {s === "all" ? "Tous" : s === "pending" ? "En attente" : s === "approved" ? "Approuvés" : s === "rejected" ? "Rejetés" : "En cours"}
                </Button>
              ))}
            </div>
                    ...
                    <CardContent className="p-4">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-sm bg-secondary/50 rounded-lg p-3 mb-3">
                        <div>
                          ...
                          <p className="font-bold text-lg">{Number(w.amount).toLocaleString()} GPB</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground text-xs">Montant net</p>
                          <p className="font-bold text-lg text-primary">{Number(w.netAmount).toLocaleString()} GPB</p>
                        </div>
                        <div>
                      ...
                      <div className="flex items-start justify-between">
                        ...
                        <div className="text-right">
                          <p className={`font-bold ${type === "deposit" ? "text-green-600" : "text-orange-600"}`}>
                            {type === "deposit" ? "+" : "-"}{Number(type === "deposit" ? item.amount : item.netAmount).toLocaleString()} GPB
                          </p>
