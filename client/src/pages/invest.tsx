...
export default function InvestPage() {
  ...
  const purchaseMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Produit acheté !", description: "Vous commencerez à recevoir vos gains demain." });
    },
    onError: (error: any) => {
      setConfirmProduct(null);
    ...
    <div className="flex flex-col min-h-full" style={{ background: "#f0f2f5" }}>
      ...
      >
          <div className="flex items-center gap-2">
            <img src={hsbcLogo} alt="HSBC logo" className="h-8 w-8 rounded-md object-cover object-left" />
            <span className="text-white text-sm font-bold">HSBC</span>
          </div>
        <button
      ...
      <div className="flex gap-3 px-4 pt-4 pb-2">
        <button
          ...
          style={{
            ...
            color: activeTab === "all" ? "#fff" : "#FF0000",
          }}
        >
          our products
        </button>
        <button
          ...
          style={{
            background: activeTab === "mine"
              ? "linear-gradient(135deg, #16a34a, #22c55e)"
              : "linear-gradient(135deg, #bbf7d0, #86efac)",
            color: activeTab === "mine" ? "#fff" : "#FF0000",
          }}
        >
          my product
        </button>
      ...
      {/* ── Product list ── */}
      <div className="flex-1 overflow-y-auto bottom-nav-clearance px-3 space-y-3 pt-1">
        {isLoading ? (
          <RefreshLoader />
        ) : displayed.length > 0 ? (
