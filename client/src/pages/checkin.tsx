...
import checkinHero from "@assets/ANIMATIONS-NUAGES0250_1789486316825.jpg";
...
export default function CheckinPage() {
  ...
  const claimMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Bonus received!", description: "204 GPB added to your balance" });
    },
    onError: (error: Error) => {
  ...
  return (
    <main className="checkin-reference min-h-full bg-[#f4f4f4] bottom-nav-clearance">
      <style>{`
        .checkin-reference {
          color: #171717;
        ...
        .checkin-reference .avatar {
          ...
          border-radius: 50%;
           background-image: url("/hsbc-favicon.png");
      ...
      <div className="checkin-screen">
        ...
        <section className="earnings-card" aria-label="Gains de pointage quotidien">
          <p className="earned-total">{formatAmount(totalBonusClaimed)}</p>
          <p className="earned-heading">Gains cumulés</p>

          <div className="stats">
            <div className="stat">
              <p className="stat-value">204<span>{currency}</span></p>
              <p className="stat-label">Gains quotidiens du pointage</p>
            </div>
            <div className="stat">
              <p className="stat-value secondary">{Math.round(totalBonusClaimed)}<span>{currency}</span></p>
              <p className="stat-label">Gains cumulés</p>
            </div>
          </div>
        </section>
